...
def bulbDiscovery()
{
	...
	return dynamicPage(name:"bulbDiscovery", title:"Group Discovery Started!", nextPage:"", refreshInterval:refreshInterval, install:true) {
...
def addBridge() {
	def vbridges = getVerifiedHueBridges()
    log.debug "BRIDGES: " + vbridges.collect{"$it.value.ip:$it.value.port"}
...
def locationHandler(evt) {
	log.trace "$locationHandler(evt.description)"
	...
	{ // HUE BRIDGE RESPONSES
		...
		{ //(application/json)
			...
			{ //GET /api/${state.username}/lights response (application/json)
				if (!body?.action?.on) { //check if first time poll made it here by mistake
					def bulbs = getHueBulbs()
